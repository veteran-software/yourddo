...
const useSubjugation = () => {
  ...
  const isAspect = (elementName: string) => {
    return (
      elementName.includes('Balance') ||
      elementName.includes('Existential') ||
      elementName === 'Tempered'
    )
  }

  const filterCallback = useCallback(
    (item: CraftingIngredient, elementName: string, elements: string[]) => {
      if (elements[0] === elements[1] && isAspect(elementName)) {
        return (
          item.effectsAdded?.some((effect: Enhancement) => {
            console.log(
              elementName,
              effect.name.includes(`Aspect of ${elementName}`)
            )
            return effect.name.includes(`Aspect of ${elementName}`)
          }) ?? false
        )
      } else {
        return (
          item.effectsAdded?.some((effect: Enhancement) =>
            effect.name.includes(elementName)
          ) ?? false
        )
      }
    },

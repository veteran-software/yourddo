...
const BrowserItem = (props: Props) => {
  ...
  return (
    <button
      ...
      onClick={() => {
        selectItem(browsingSlot, item)
      }}
    >
      <div className='w-100'>
        <div className='d-flex align-items-center gap-1 mb-0 small'>
          {currentEquipped.some((e) => e.id === item.id) && <GenericBadge badgeText='Equipped' />}

          {troveData !== null && <TroveBadge itemName={item.name} troveData={troveData} />}
        </div>
        ...
        {item.setBonus && item.setBonus.length > 0 && (
          <div className='mt-1 mb-1'>
            {item.setBonus.map((sb) => (
              <SetBonusBadge key={sb.name} openSetBonusBrowser={openSetBonusBrowser} setName={sb.name} />
            ))}
          </div>
        )}
